...
import { SentimentRequestBody, SentimentResult } from "@/types/sentiment";
...
const Index = () => {
  ...
  const handleAppNameInputChange = (event: ChangeEvent<HTMLInputElement>) => {
  ...
  const handleAppReviewLimitInputChange = (event: ChangeEvent<HTMLInputElement>) => {
    console.log(event.target.value);
    if (event.target.value === "") {
      setReviewLimit(null);
      return;
    }
    setReviewLimit(parseInt(event.target.value));
  };
  ...
  const handleSubmit = async () => {
    ...
    const body: SentimentRequestBody = { appName };

    if (reviewLimit !== null && reviewLimit !== undefined) {
      body.reviewLimit = reviewLimit;
    }
    ...
    try {
      ...
      if (response.ok) {
        ...
        setError(data.detail[0].msg || data.detail || "Something went wrong.");
      }
    } catch (error) {
      ...
      <div className="relative mx-auto max-w-4xl grid space-y-5 sm:space-y-10">
        <div className="text-center">
          ...
          <h1 className="text-3xl text-gray-800 font-bold sm:text-5xl lg:text-6xl lg:leading-tight dark:text-neutral-200">
            Uncover users true feelings  <span className="text-blue-500">Analyze sentiment effortlessly!</span>
          </h1>
        </div>
        ...
        <form>
          <div className="mx-auto max-w-2xl sm:flex sm:space-x-3 p-3 bg-white border border-gray-200 rounded-lg shadow-lg shadow-gray-100 dark:bg-neutral-900 dark:border-neutral-700 dark:shadow-gray-900/20">
            <div className="w-full pb-2 sm:pb-0">
              <label className="block text-sm font-medium dark:text-white"><span className="sr-only">App name</span></label>
              <input 
                type="text" 
                id="play-store-app-name" 
                className="py-2.5 sm:py-3 px-4 block w-full border-transparent rounded-lg sm:text-sm focus:border-blue-500 focus:ring-blue-500 dark:bg-neutral-900 dark:border-transparent dark:text-neutral-400 dark:placeholder-neutral-500 dark:focus:ring-neutral-600" 
                placeholder="App name"
                value={appName}
                onChange={handleAppNameInputChange}
              />
              {suggestions.length > 0 && !isFetchingSuggestions && (
                <ul className="mt-2 bg-white border border-gray-200 rounded-lg shadow-lg dark:bg-neutral-800 dark:border-neutral-600 max-h-60 overflow-y-auto z-100 absolute">
            ...
            <div className="pt-2 sm:pt-0 sm:ps-3 border-t border-gray-200 sm:border-t-0 sm:border-s w-full dark:border-neutral-700">
              <label className="block text-sm font-medium dark:text-white"><span className="sr-only">Limit</span></label>
              <input 
                type="number" 
                id="play-store-review-limit" 
                className="py-2.5 sm:py-3 px-4 block w-full border-transparent rounded-lg sm:text-sm focus:border-blue-500 focus:ring-blue-500 dark:bg-neutral-900 dark:border-transparent dark:text-neutral-400 dark:placeholder-neutral-500 dark:focus:ring-neutral-600" 
                placeholder="Limit"
                value={reviewLimit || ""}
                onChange={handleAppReviewLimitInputChange}
              />
            </div>
            <div 
              className="whitespace-nowrap pt-2 sm:pt-0 grid sm:block"
        ...
        {results && (
            ...
            <div className="text-center">
                ...
                Sentiment Analysis Results for {results.app_name}
                </h2>
            </div>
            <div className="bg-white p-6 rounded-lg shadow-md dark:bg-neutral-800 dark:text-white">
                <div className="space-y-3">
                <p className="text-lg font-medium">
                    Overall Sentiment:{" "}
                    <span className="font-bold">{results.average_score}</span>
                </p>
                <p className="text-md">
                    Number Of Reviews:{" "}
                    <span className="font-bold">{results.review_count}</span>
                </p>
                </div>
            </div>
            </div>
